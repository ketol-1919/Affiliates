...
const HomeScreen: React.FC<HomeScreenProps> = ({ posts, onNavigateToCreator, onToggleLike }) => {
  ...
  const filteredPosts = posts.filter(post => {
    if (activeFilter === 'image') return post.type === 'image';
    if (activeFilter === 'video') return post.type === 'video';
    if (activeFilter === 'liked') return post.liked;
    return true; // for 'all'
  });
  ...
  return (
    <div className="w-full h-full relative flex-grow flex flex-col">
      {posts.length === 0 ? (
        ...
        <>
          <div className="flex flex-wrap items-center justify-center gap-2 md:gap-4 mb-8 animate-fade-in">
